...
class gmapViewGmp extends viewGmp {
	...
	public function drawMap($params) {
		...
		if(isset($params['plugin-info']) && $params['plugin-info'] == 'Membership-by-Supsystic' && isset($params['membership-params'])) {
			$membershipModel = frameGmp::_()->getModule('membership')->getModel('membership_presets');
	...
	public function getEditMap($id = 0) {
		...
		if(!$membershipModel) {
			...
			$this->assign('membershipPluginError', __('You need to reactivate your Google Maps Easy plugin.'), GMP_LANG_CODE );
		} else {
